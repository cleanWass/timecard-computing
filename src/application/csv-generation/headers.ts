const addPrefix = <T extends readonly string[], P extends string>(
  ...
  arr.map(day => `${prefix} ${day}` as `${P} ${typeof day}`) as { [K in keyof T]: `${P} ${T[K] & string}` };
...
export const SilaeHeaders = [...EmployeeHeaders, ...ContractHeaders, ...WorkedHoursHeaders, ...PerksHeaders] as const;
...
export const TotalHeaders = [...EmployeeHeaders, ...ContractHeaders, ...WorkedHoursHeaders, ...PerksHeaders] as const;
