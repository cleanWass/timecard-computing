  ...
  timecard.with({
    mealTickets: pipe(timecard, tc => {
      ...
      return shiftsByDay.reduce((tickets, shiftsOfDay) => {
        ...
        const hasNoMorningShift = !shiftsOfDay.some(shift =>
          shift.getStartTime().toLocalTime().isBefore(noon)
        );
        const hasNoEveningShift = !shiftsOfDay.some(shift =>
          shift.getEndTime().toLocalTime().isAfter(noon)
        );

        if (hasNotSeniority || hasNotEnoughHours || hasNoMorningShift || hasNoEveningShift)
          return tickets;

        const shiftsDuringBreakLunch = shiftsOfDay.filter(shift =>
          shift.getTimeSlot().overlaps(breakLunch)
        );
