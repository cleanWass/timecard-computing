...
function findMostRecentCsvFile(baseDir: string, month: string, format: string): string | null {
  try {
    ...
    const timeDirs = fs.readdirSync(dateDir)
...
function getAvailableMonths(year: string, format: string): string[] {
  ...
  const monthsInBaseDir = fs.readdirSync(baseDir)
    .filter(dir => !dir.startsWith('.')) // Skip hidden files
  ...
  const monthsInBaseDirNew = fs.readdirSync(baseDirNew)
    .filter(dir => !dir.startsWith('.')) // Skip hidden files
    .filter(dir => fs.statSync(path.join(baseDirNew, dir)).isDirectory())
    .filter(dir => MONTHS.includes(dir.toLowerCase())); // Only include valid month names
  
  // Find months that exist in both directories and have CSV files
  const availableMonths = monthsInBaseDir.filter(month => {
 ...
 */
function compareCSVMonth(year: string, month: string, format: string): { output: string, differences: any } {
  const baseDir = path.join('exports', year);
  const baseDirNew = path.join('exports', `${year}_NEW`);
 ...
 */
function parseComparisonOutput(output: string): { count: number, silaeIds: string[], details: any[] } {
...
function compareCSVYear(year: string, format: string): void {
  ...
  console.log(chalk.default.yellow(`Found ${availableMonths.length} months for comparison: ${availableMonths.join(', ')}`));
