...
import { employeeRole } from '../../domain/models/employee-registration/employee/employee-role';
...
const periodSchema = zod.object({
  start: zod.string(),
  end: zod.string().nullish(),
});
const closedPeriodSchema = zod.object({
  start: zod.string(),
  end: zod.string(),
});
...
const employeeWithTimecardSchema = zod
  ...
  .transform((raw) => {
    const contractPlanningsGroupedByContractId = raw.plannings.reduce(
      (map, curr) =>
        map.update(curr.contract.id, List<[PlanningSchema, ClosedPeriodSchema]>(), (list) =>
